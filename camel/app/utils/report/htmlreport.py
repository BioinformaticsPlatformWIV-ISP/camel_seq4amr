...
class HtmlReport(HtmlBase):
    ...
    def add_pipeline_header(self, pipeline_name: str) -> None:
        ...
        :return: None
        """
        path_logo = Path(pkg_resources.resource_filename('camel', 'resources/logo-sciensano.png'))
        if self._output_dir is None:
        ...
        with self.get_tag('div', [('class', 'header')]):
            with self.get_tag('div', [('id', 'header_title')]):
                self._doc.stag('img', src=path_logo.name, alt='Sciensano Belgium', id='header_logo',
                               height=80)
                self.add_text(f"{pipeline_name} report")
        shutil.copy(path_logo, self._output_dir)
